...
def distanceMetric(thing_A, thing_B):
    '''
    A "universal distance" metric that can be used as a default in many
    settings.
    ...
    if typeA is list and typeB is list:
        ...
        # If both inputs are array-like, return the maximum absolute difference
        # b/w corresponding elements (if same shape); return largest difference
        # in dimensions if shapes do not align.
...
class HARKobject(object):
    ...
    def distance(self, other):
        ...
        called distance_criteria, giving a list of strings naming the
        attributes to be considered by the distance metric.
    ...
    def __call__(self, **kwds):
        '''
        Assign an arbitrary number of attributes to this agent, as a
        convenience.  See assignParameters.
        '''
        self.assignParameters(**kwds)

    def getAvg(self, varname, **kwds):
        '''
        Calculates the average of an attribute of this instance.  Returns NaN
        if no such attribute.

        Parameters
        ----------
        varname : string
            The name of the attribute whose average is to be calculated.  This
            attribute must be an np.array or other class compatible with
            np.mean.

        Returns
        -------
        avg : float or np.array
            The average of this attribute.  Might be an array if the axis
            keyword is passed.
        '''
...
class Solution(HARKobject):
    '''
    A superclass for representing the "solution" to a single period problem in
    a dynamic microeconomic model.
...
class AgentType(HARKobject):
    ...
    specify its own subclass of AgentType, inheriting its methods and
    overwriting as necessary.  Critically, every subclass of AgentType should
    define class- specific static values of the attributes time_vary and
    time_inv as lists of strings.  Each element of time_vary is the name of a
    field in AgentSubType that varies over time in the model.  Each element of
    time_inv is the name of a field in AgentSubType that is constant over time
    in the model. The string 'solveOnePeriod' should appear in exactly one of
    these lists, depending on whether the same solution method is used in all
    periods of the model.
    '''

    def __init__(self, solution_terminal=None, cycles=1, time_flow=False,
                 pseudo_terminal=True, tolerance=0.000001, seed=0, **kwds):
        '''
        ...
        solution_terminal : Solution
            ...
            this AgentType instance, or an initial guess of the solution if
            this is an infinite horizon problem.
        cycles : int
            ...
            once before terminating.  cycles=0 corresponds to an infinite
            horizon model, with a sequence of one period problems repeating
            indefinitely.
        time_flow : boolean
        ...
        tolerance : float
            ...
            for the solution to be considered as having "converged".
            Inoperative when cycles>0.
        seed : int
    ...
    def timeReport(self):
        ...
        '''
        if self.time_flow:
            print('Time varying objects are listed in ordinary chronological '
                  'order.')
        else:
            print('Time varying objects are listed in reverse chronological '
                  'order.')
    ...
    def delFromTimeVary(self, *params):
        ...
        ----------
        params : string
            Any number of strings naming attributes to be removed from
            time_vary
    ...
    def solve(self, verbose=False):
        '''
        Solve the model for this instance of an agent type by backward
        induction.  Loops through the sequence of one period problems, passing
        the solution from period t+1 to the problem for period t.
        ...
        # Do pre-solution stuff
        self.preSolve()
        # Solve the model by backward induction
        self.solution = solveAgent(self, verbose)
        # Put the solution in chronological order if this instance's time flow
        # runs that way
        if self.time_flow:
            self.solution.reverse()
        # Add solution to the list of time-varying attributes
        self.addToTimeVary('solution')
        # Do post-solution stuff
        self.postSolve()
    ...
    def checkElementsOfTimeVaryAreLists(self):
        ...
        for param in self.time_vary:
            assert type(getattr(self, param)) == list, \
                param + ' is not a list, but should be because it is in ' \
                'time_vary'

    def preSolve(self):
        '''
        A method that is run immediately before the model is solved, to check
        inputs or to prepare the terminal solution, perhaps.
    ...
    def initializeSim(self):
        '''
        Prepares this AgentType for a new simulation.  Resets the internal
        random number generator, makes initial states for all agents (using
        simBirth), clears histories of tracked variables.
    ...
    def simOnePeriod(self):
        '''
        Simulates one period for this type.  Calls the methods getMortality(),
        getShocks() or readShocks, getStates(), getControls(), and
        getPostStates().  These should be defined for AgentType subclasses,
        except getMortality (define its components simDeath and simBirth
        ...
        None
        '''
        # Replace some agents with "newborns"
        self.getMortality()
        # If shock histories have been pre-specified, use those
        if self.read_shocks:
            self.readShocks()
        # Otherwise, draw shocks as usual according to subclass-specific method
        else:
            self.getShocks()
        # Determine each agent's state at decision time
        self.getStates()
        # Determine each agent's choice or control variables based on states
        self.getControls()
        # Determine each agent's post-decision / end-of-period states using
        # states and controls
        self.getPostStates()

        # Advance time for all agents
        # Age all consumers by one period
        self.t_age = self.t_age + 1
        # Age all consumers within their cycle
        self.t_cycle = self.t_cycle + 1
        # Resetting to zero for those who have reached the end
        self.t_cycle[self.t_cycle == self.T_cycle] = 0

    def makeShockHistory(self):
        '''
        Makes a pre-specified history of shocks for the simulation.  Shock
        variables should be named in self.shock_vars, a list of strings that is
        subclass-specific.  This method runs a subset of the standard
        simulation loop by simulating only mortality and shocks; each variable
        named in shock_vars is stored in a T_sim x AgentCount array in an
        attribute of self named X_hist.  Automatically sets self.read_shocks to
        True so that these pre-specified shocks are used for all subsequent
        calls to simulate().
        ...
        for t in range(self.T_sim):
            ...
            self.getShocks()
            for var_name in self.shock_vars:
                exec('self.' + var_name + '_hist[self.t_sim,:] = self.'
                     + var_name)
            self.t_sim += 1
            # Age all consumers by one period
            self.t_age = self.t_age + 1
        ...
        # Restore the flow of time and flag that shocks can be read rather than
        # simulated
    ...
    def getMortality(self):
        '''
        Simulates mortality or agent turnover according to some model-specific
        rules named simDeath and simBirth (methods of an AgentType subclass).
        simDeath takes no arguments and returns a Boolean array of size
        AgentCount, indicating which agents of this type have "died" and must
        be replaced.  simBirth takes such a Boolean array as an argument and
        generates initial post-decision states for those agent indices.
    ...
    def simDeath(self):
        '''
        Determines which agents in the current population "die" or should be
        replaced.  Takes no inputs, returns a Boolean array of size
        self.AgentCount, which has True for agents who die and False for those
        that survive. Returns all False by default, must be overwritten by a
        ...
        -------
        who_dies : np.array
            Boolean array of size self.AgentCount indicating which agents die
            and are replaced.
        '''
    ...
    def simBirth(self, which_agents):
        '''
        Makes new agents for the simulation.  Takes a boolean array as an
        input, indicating which agent indices are to be "born".  Does nothing
        by default, must be overwritten by a subclass.
    ...
    def getShocks(self):
        '''
        Gets values of shock variables for the current period.  Does nothing by
    ...
    def readShocks(self):
        '''
        Reads values of shock variables for the current period from history
        arrays.  For each variable X named in self.shock_vars, this attribute
        of self is set to self.X_hist[self.t_sim,:].
        ...
        for var_name in self.shock_vars:
            setattr(self, var_name,
                    getattr(self, var_name+'_hist')[self.t_sim, :])

    def getStates(self):
        '''
        Gets values of state variables for the current period, probably by
        using post-decision states from last period, current period shocks, and
        maybe market-level events.  Does nothing by default, but can be
        overwritten by subclasses of AgentType.
    ...
    def getControls(self):
        '''
        Gets values of control variables for the current period, probably by
        using current states.  Does nothing by default, but can be overwritten
        by subclasses of AgentType.
    ...
    def getPostStates(self):
        ...
        probably by current states and controls and maybe market-level events
        or shock variables.  Does nothing by default, but can be overwritten by
        subclasses of AgentType.
    ...
    def simulate(self, sim_periods=None):
        ...
        self.T_sim if no input).  Records histories of attributes named in
        self.track_vars in attributes named varname_hist.
    ...
    def clearHistory(self):
        ...
        for var_name in self.track_vars:
            exec('self.' + var_name
                 + '_hist = np.zeros((self.T_sim,self.AgentCount)) + np.nan')


def solveAgent(agent, verbose):
    '''
    Solve the dynamic model for one agent type.  This function iterates on
    "cycles" of an agent's model either a given number of times or until
    solution convergence if an infinite horizon model is used (with
    agent.cycles = 0).
    ...
    '''
    # Record the flow of time when the Agent began the process, and make sure
    ...
    # Initialize the solution, which includes the terminal solution if it's not
    # a pseudo-terminal period
    solution = []
    ...
    while go:
        ...
        # Check for termination: identical solutions across cycle iterations or
        # run out of cycles
        solution_now = solution_cycle[-1]
        if infinite_horizon:
            if completed_cycles > 0:
                solution_distance = solution_now.distance(solution_last)
                go = (solution_distance > agent.tolerance
                      and completed_cycles < max_cycles)
            # Assume solution does not converge after only one cycle
            else:
        ...
        if verbose:
            t_now = clock()
            if infinite_horizon:
                print('Finished cycle #' + str(completed_cycles) + ' in '
                      + str(t_now-t_last) + ' seconds, solution distance = '
                      + str(solution_distance))
            else:
                print('Finished cycle #' + str(completed_cycles) + ' of '
                      + str(agent.cycles) + ' in ' + str(t_now-t_last)
                      + ' seconds.')
            t_last = t_now

    # Record the last cycle if horizon is infinite (solution is still empty!)
    if infinite_horizon:
        # PseudoTerminal=False impossible for infinite horizon
        solution = solution_cycle

    # Restore the direction of time to its original orientation, then return
    # the solution
...
def solveOneCycle(agent, solution_last):
    ...
    iterates over the periods within an agent's cycle, updating the
    time-varying parameters and passing them to the single period solver(s).
    ...
    '''
    # Calculate number of periods per cycle, defaults to 1 if all variables are
    # time invariant
...
class Market(HARKobject):
    ...
    def __init__(self, agents=[], sow_vars=[], reap_vars=[], const_vars=[],
                 track_vars=[], dyn_vars=[], millRule=None, calcDynamics=None,
                 act_T=1000, tolerance=0.000001):
        '''
        ...
        sow_vars : [string]
            Names of variables generated by the "aggregate market process" that
            should be "sown" to the agents in the market.  Aggregate state,
            etc.
        reap_vars : [string]
            Names of variables to be collected ("reaped") from agents in the
            market to be used in the "aggregate market process".
        const_vars : [string]
            Names of attributes of the Market instance that are used in the
            "aggregate market process" but do not come from agents-- they are
            constant or simply parameters inherent to the process.
        track_vars : [string]
            Names of variables generated by the "aggregate market process" that
            should be tracked as a "history" so that a new dynamic rule can be
            calculated.  This is often a subset of sow_vars.
        dyn_vars : [string]
            Names of variables that constitute a "dynamic rule".
        millRule : function
            A function that takes inputs named in reap_vars and returns an
            object with attributes named in sow_vars.  The "aggregate market
            process" that transforms individual agent actions/states/data into
            aggregate data to be sent back to agents.
        calcDynamics : function
            A function that takes inputs named in track_vars and returns an
            object with attributes named in dyn_vars.  Looks at histories of
            aggregate variables and generates a new "dynamic rule" for agents
            to believe and act on.
        act_T : int
            The number of times that the "aggregate market process" should be
            run in order to generate a history of aggregate variables.
        tolerance: float
            Minimum acceptable distance between "dynamic rules" to consider the
            Market solution process converged.  Distance is a user-defined
            metric.
    ...
    '''
        ...
        # To prevent overwriting of method-based millRules
        if millRule is not None:
            self.millRule = millRule
        # Ditto for calcDynamics
        if calcDynamics is not None:
        ...
        # "solveAgents" one time. If set to false, the error will never print.
        # See "solveAgents" for why this prints once or never.
    ...
    def solveAgents(self):
        ...
        try:
            ...
            if self.print_parallel_error_once:
                ...
                print("**** WARNING: could not execute multiThreadCommands "
                      "in HARK.core.Market.solveAgents(), so using the "
                      "serial version instead. This will likely be slower. "
                      "The multiTreadCommands() functions failed with the "
                      "following error:", '\n    ',
    ...
    def solve(self):
        '''
        "Solves" the market by finding a "dynamic rule" that governs the
        aggregate market state such that when agents believe in these dynamics,
        their actions collectively generate the same dynamic rule.
        ...
        while go:
            ...
            new_dynamics = self.updateDynamics()

            # Check to see if the dynamic rule has converged (if this is not
            # the first loop)
        ...
        # Store the final dynamic rule in self
        self.dynamics = new_dynamics

    def reap(self):
        '''
        Collects attributes named in reap_vars from each AgentType in the
        market, storing them in respectively named attributes of self.
    ...
    def mill(self):
        '''
        Processes the variables collected from agents using the function
        millRule, storing the results in attributes named in aggr_sow.
    ...
    def reset(self):
        ...
        user-defined initial state, and erase the histories of tracked
        variables.
    ...
    def makeHistory(self):
        ...
        evolution of variables X named in track_vars in attributes named
        X_hist.
        ...
        for t in range(self.act_T):
            ...
            self.reap()
            # Process individual data into aggregate data
            self.mill()
    ...
    def updateDynamics(self):
        '''
        Calculates a new "aggregate dynamic rule" using the history of
        variables named in track_vars, and distributes this rule to AgentTypes
        in agents.
        ...
        # Calculate a new dynamic rule and distribute it to the agents in
        # agent_list

        # User-defined dynamics calculator
...
def main():
    ...
    print("To see some examples of its frameworks in action, try running a "
          "model module.")
    print("Several interesting model modules can be found in "
          "/ConsumptionSavingModel.")
    print('For an extraordinarily simple model that demonstrates the '
          '"microeconomic" and')
