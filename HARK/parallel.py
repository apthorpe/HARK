'''
Early version of multithreading in HARK. To use most of this module, you should
first install dill and joblib.  Packages can be installed by typing
"conda install dill" (etc) at a command prompt.
'''
...
# We want to be able to import this module even if joblib and dill are not
# installed.  If we can't import joblib and dill, define the functions we tried
# to import such that they will raise useful errors if called.
def raiseImportError(moduleStr):
    def defineImportError(*args, **kwargs):
        raise ImportError(moduleStr + ' could not be imported, and is '
                          'required for this function.  See HARK '
                          'documentation for more information on how to '
                          'install the ' + moduleStr + ' module.')
...
def multiThreadCommandsFake(agent_list, command_list, num_jobs=None):
    '''
    Executes the list of commands in command_list for each AgentType in
    agent_list in an ordinary, single-threaded loop.  Each command should be a
    method of that AgentType subclass.  This function exists so as to easily
    disable multithreading, as it uses the same syntax as multithreadCommands.
...
def multiThreadCommands(agent_list, command_list, num_jobs=None):
    '''
    Executes the list of commands in command_list for each AgentType in
    agent_list using a multithreaded system. Each command should be a method of
    that AgentType subclass.
    ...
    # Send each command in command_list to each of the types in agent_list to
    # be run
...
def runCommands(agent, command_list):
    ...
    -------
    agent : AgentType
        The same AgentType instance passed as input, after running the
        commands.
    '''
    for command in command_list:
        exec('agent.' + command)
...
def parallelNelderMead(objFunc, guess, perturb=None, P=1, ftol=0.000001,
                       xtol=0.00000001, maxiter=np.inf, maxeval=np.inf,
                       r_param=1.0, e_param=1.0, c_param=0.5, s_param=0.5,
                       maxcores=None, name=None, resume=False, savefreq=None,
                       verbose=1):
    '''
    ...
    ----------
    objFunc : function
        The objective function to be minimized. Takes a single 1D array as
        input.
    guess : np.array
        Initial starting point for the simplex, representing an input for
        objFunc.
    perturb : np.array
        ...
        default, guess=None, indicating that all parameters should be
        optimized, with an initial perturbation of 0.1*guess.
    P : int
    ...
    maxeval : int
        Maximum number of evaluations of objFunc (across all processes);
        reaching evals=maxeval is reported as an "unsuccessful" minimization.
    r_param: float
    ...
    name : string
        A filename for (optionally) saving the progress of the Nelder-Mead
        search, and for resuming a previous search (when resume=True).  Useful
        for long searches that could potentially be interrupted by computer
            down time.
    resume : boolean
        An indicator for whether the search should resume from earlier
        progress.  When True, the process will load a progress file named in
        input name.
    savefreq : int
    ...
    # Otherwise, construct the initial simplex and array of function values
        ...
        # Default: perturb each parameter by 10%
        if perturb is None:
            perturb = 0.1*guess
            guess[guess == 0] = 0.1
        # Indices of which parameters to optimize
        params_to_opt = np.where(perturb != 0)[0]
        # Number of parameters to search over
        dim_count = params_to_opt.size
        # Number of points in simplex
        N = dim_count+1
        # Total number of parameters
        K = guess.size
        ...
        for j in range(dim_count):
            simplex[j+1, params_to_opt[j]] = simplex[j+1, params_to_opt[j]] \
    ...
    # Cap the number of cores if desired
    ...
    if not resume:
        ...
        temp_simplex = list(simplex)
        fvals = np.array(parallel(delayed(objFunc)(params)
                         for params in temp_simplex))
        evals += N
        ...
        x_dist = np.max(np.sqrt(np.sum(simplex**2.0 - np.tile(simplex[0, :],
                                       (N, 1))**2.0, axis=1)))
        if verbose > 0:
            print('Evaluated the initial simplex: fmin=' + str(fmin)
                  + ', f_dist=' + str(f_dist) + ', x_dist=' + str(x_dist))
    else:
        # Resume an existing search that was cut short
        if verbose > 0:
    ...
    while go:
        ...
        output = parallel(delayed(parallelNelderMeadWorker)
                          (objFunc, simplex, fvals, j, P, opt_params)
                          for j in j_list)
        ...
        if np.max(np.abs(new_subsimplex - old_subsimplex)) == 0:
            ...
            # If every attempted update was unsuccessful, must shrink the
            # simplex
            simplex = s_param * np.tile(simplex[0, :], (N, 1)) \
                + (1.0 - s_param) * simplex
            temp_simplex = list(simplex[1:N, :])
            fvals = np.array([fvals[0]] + parallel(delayed(objFunc)(params)
                             for params in temp_simplex))
            new_evals += N-1
            evals += N-1
        else:
        ...
        fmin = fvals[0]
        f_dist = np.abs(fmin - fvals[-1])
        x_dist = np.max(np.sqrt(np.sum(simplex**2.0
                                       - np.tile(simplex[0, :], (N, 1))**2.0,
                                       axis=1)))
        t_end = clock()
        if verbose > 0:
...
def parallelNelderMeadWorker(objFunc, simplex, f_vals, j, P, opt_params):
    ...
    simplex : numpy.array
        The current simplex for minimization; simplex[k,:] is an input for
        objFunc.
    f_vals : numpy.array
    ...
    opt_params : numpy.array
        Three element array with parameters for reflection, contraction,
        expansion.
    ...
    # vertex to update
    my_point = simplex[j, :]
    # value at the vertex to update
    my_val = f_vals[j]
    # best value in the vertex
    best_val = f_vals[0]
    # next best point in the simplex
    next_val = f_vals[j-1]
    evals = 0
    ...
    # Case 2: the reflection point is better than the next best point
        ...
        # Report reflection point
        new_point = r_point
        new_val = r_val
    # Case 3: the reflection point is worse than the next best point
    else:
        if r_val < my_val:
            # Check whether reflection or original point is better and use it
            # temporarily
            temp_point = r_point
            temp_val = r_val
        else:
        ...
        c_val = objFunc(c_point)
        evals += 1
        if c_val < temp_val:
            new_point = c_point
            # Check whether the contraction point is better
            new_val = c_val
        else:
...
def main():
    ...
    print("To see an example of multithreading in HARK, "
          "see /Testing/MultithreadDemo.")
    print('To ensure full compatibility "out of the box", '
          'multithreading is not')
    print('used in our models and applications; users can turn it on by '
          'modifying')
    ...
    xopt, fmin = parallelNelderMead(testFunc1, my_guess, P=P, maxiter=300,
                                    savefreq=100, name='testfile',
                                    resume=False)
